# The Repository of Yehor Panasenko: A Compilation of Research and Resources


```cardlink
url: https://github.com/gaurapanasenko/
title: "gaurapanasenko - Overview"
description: "gaurapanasenko has 31 repositories available. Follow their code on GitHub."
host: github.com
favicon: https://github.githubassets.com/favicons/favicon.svg
image: https://avatars.githubusercontent.com/u/16933933?v=4?s=400
```


```cardlink
url: https://www.youtube.com/channel/UCyx9fVMNihGPVwTly3cSdLw
title: "Yehor Panasenko"
description: "Share your videos with friends, family, and the world"
host: www.youtube.com
favicon: https://www.youtube.com/s/desktop/e4367744/img/favicon_32x32.png
image: https://yt3.googleusercontent.com/ytc/AIdro_lsvjF3Bvcho5s9P4xniJDkS6EJ2mWn5R6Y2fY2ww=s900-c-k-c0x00ffffff-no-rj
```


```cardlink
url: https://github.com/gaurapanasenko/
title: "Yehor Panasenko | LinkedIn"
host: linkedin.com
favicon: https://static.licdn.com/aero-v1/sc/h/3loy7tajf3n0cho89wgg0fjre
image: https://media.licdn.com/dms/image/C4D03AQGIprzKM3-iuA/profile-displayphoto-shrink_800_800/0/1601459171552?e=1715817600&v=beta&t=f-8CGo03LjBjGniszdWsp3Idv76y6JY7Z-7Df_s9I60
```

```cardlink
url: https://gitlab.com/gaura.panasenko/eegzen
title: "Yehor Panasenko / EegZen · GitLab"
description: "GitLab.com"
host: gitlab.com
favicon: https://gitlab.com/assets/favicon-72a2cad5025aa931d6ea56c3201d1f18e68a8cd39788c7c80d5b2b82aa5143ef.png
image: https://gitlab.com/assets/twitter_card-570ddb06edf56a2312253c5872489847a0f385112ddbcd71ccfa1570febab5d2.jpg
```


```cardlink
url: https://gaurapanasenko.github.io/export/science_works/
title: "Science works"
description: "Several interesting science works from students"
host: gaurapanasenko.github.io
favicon: https://gaurapanasenko.github.io/favicon.png
image: https://gaurapanasenko.github.io/science_works.png
```


```cardlink
url: https://github.com/gaurapanasenko/python3-thinkgear
title: "GitHub - gaurapanasenko/python3-thinkgear: ThinkGear Serial Stream Protocol implementation"
description: "ThinkGear Serial Stream Protocol implementation."
host: github.com
favicon: https://github.githubassets.com/favicons/favicon.svg
image: https://opengraph.githubassets.com/3fb4b98dba9137a17d56e33639afeee32531c84fd826ab70a407a3867a7da660/gaurapanasenko/python3-thinkgear
```


```cardlink
url: http://om.dp.ua/
title: "На кухне у Ами | кушаем вкусно :) вегетарианские рецепты"
description: "Вегетарианские рецепты, рецепты в пароварке, сыроедение"
host: om.dp.ua
favicon: http://om.dp.ua/sites/om.dp.ua/files/clear_dark_favicon.png
image: http://elfiny.top/software/Om.png
```


```cardlink
url: http://recipe.elfiny.top
title: "Все рецепты"
description: "Лучшие проверенные вегетарианские рецепты с расчетом калорий и питательных веществ."
host: recipe.elfiny.top
favicon: http://recipe.elfiny.top/static/img/icon/favicon-32x32.png
image: https://gaurapanasenko.github.io/recipe_elfiny_top.png
```


```cardlink
url: https://elfiny.top/socionic/
title: "Socionic Cube"
description: "Simple socionic cube"
host: elfiny.top
favicon: http://elfiny.top/images/logo.png
image: http://elfiny.top/software/SocionicCube.png
```


